...
requester_executable="${build_directory}/Build/Products/Debug/WhoSudodLiveRequester.app/Contents/MacOS/WhoSudodLiveRequester"
waiter="${tool_directory}/wait-for-live-tree.swift"
temporary_directory="$(mktemp -d /private/tmp/who-sudod-live.XXXXXX)"
...
requester_log="${temporary_directory}/requester.log"
...
cleanup() {
    local exit_code="$?"
    trap - EXIT INT TERM
    stop_expected_process_tree "${requester_process_id}" "${requester_executable}"
    ...
    if (( exit_code != 0 )) && {
        ...
        /bin/cp -f "${requester_log}" "${result_directory}/" 2>/dev/null || true
    ...
    if [[ "${temporary_directory}" == /private/tmp/who-sudod-live.* ]]; then
...
for attempt in {1..50}; do
    ...
    if ! kill -0 "${requester_process_id}" 2>/dev/null; then
        print -u2 "The requester exited early. Its private log will be saved with the failure artifacts."
        exit 70
    fi
    /bin/sleep 0.1
done
if [[ ! -f "${expected_path}" ]]; then
    print -u2 "The requester did not write its expected tree."
    exit 70
fi
...
print "PASS ${request_mode} live process tree"
